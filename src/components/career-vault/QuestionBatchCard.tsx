            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                  ...
                  >
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {question.options.map((option) => (
                        <div key={option.value} className="flex items-center space-x-2 p-3 border border-border rounded bg-secondary dark:bg-muted hover:bg-accent transition-colors cursor-pointer group">
                          <RadioGroupItem value={option.value} id={`${question.id}-${option.value}`} />
                          <Label htmlFor={`${question.id}-${option.value}`} className="cursor-pointer flex items-center gap-2 flex-1 text-foreground group-hover:text-accent-foreground">
                ...
                {question.inputType === 'checkbox_grid' && question.options && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {question.options.map((option) => (
                      <div key={option.value} className="flex items-center space-x-2 p-3 border border-border rounded bg-secondary dark:bg-muted hover:bg-accent transition-colors group">
                        <Checkbox
                  ...
                  <div className="space-y-3">
                    ...
                    >
                      <div className="flex gap-3">
                        <div className="flex items-center space-x-2 p-3 border border-border rounded bg-secondary dark:bg-muted hover:bg-accent transition-colors cursor-pointer flex-1 group">
                          <RadioGroupItem value="yes" id={`${question.id}-yes`} />
                          <Label htmlFor={`${question.id}-yes`} className="cursor-pointer flex-1 text-foreground group-hover:text-accent-foreground">Yes</Label>
                        </div>
                        <div className="flex items-center space-x-2 p-3 border border-border rounded bg-secondary dark:bg-muted hover:bg-accent transition-colors cursor-pointer flex-1 group">
                          <RadioGroupItem value="no" id={`${question.id}-no`} />
                          <Label htmlFor={`${question.id}-no`} className="cursor-pointer flex-1 text-foreground group-hover:text-accent-foreground">No</Label>
                        </div>
                      </div>
                    </RadioGroup>
