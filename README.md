...
The Evidence-Based Resume Builder transforms the resume creation process from generative AI to **AI-assisted evidence mapping**. Instead of hallucinating content, the system matches job requirements to verified evidence from the user's Career Vault.

### Core Components

1.  **Evidence Engine (Backend)**
    *   `match-requirements-to-bullets` (Edge Function): Semantic matching engine.
    *   `resume_requirement_mappings` (Database): Audit trail of evidence decisions.
    *   `generate-dual-resume-section` (Edge Function): Refactored to accept evidence matrices.

2.  **Proof Builder (Frontend)**
    *   `RequirementBulletMapper`: 3-column UI for reviewing and approving evidence.
    *   `SwapEvidenceDialog`: Modal for choosing alternative evidence.
    *   `SectionEditorPanel`: Orchestrator for the section generation workflow.

3.  **Traceability Layer**
    *   `ResumeBulletMetadata`: Interface for tracking source, match score, and keywords.
    *   `LiveResumeCanvas`: Visual resume preview with hover tooltips showing evidence sources.

## User Flow

1.  **Job Analysis**: User pastes a job description.
2.  **Evidence Matching**: AI scans the Career Vault to find the best proof for each requirement.
3.  **Review & Approve**: User reviews the "Evidence Matrix" (Requirement -> Proof -> Result).
4.  **Generation**: AI assembles the resume section using *only* approved evidence.
5.  **Verification**: User can hover over any bullet in the final resume to see its source.
...
interface EvidenceMatch {
  requirementId: string;
  originalBullet: string; // From Vault
  enhancedBullet: string; // Optimized for Resume
...
## Key Features
*   **Zero Hallucination**: Every claim is linked to a database record.
*   **Full Traceability**: "Hover to Verify" feature.
*   **User Control**: Users can swap evidence or edit custom versions.
*   **ATS Optimization**: Explicit tracking of added keywords.
